...
const ExchangeReservationDetailContent = defineAsyncComponent(() =>
  import("~/components/dashboard/pages/reservations/manage/components/manage/ExchangeReservationDetailContent.vue"),
);
const ExchangeReservationConfirmContent = defineAsyncComponent(() =>
  import("~/components/dashboard/pages/reservations/manage/components/manage/ExchangeReservationConfirmContent.vue"),
);
...
const confirmExchange = async () => {
  ...
  busy.value = true;
  try {
    const payload = buildChangeProductPayload({
      newProductId: newProductId.value,
      ...(priceComparison.value?.kind === "less"
        ? {
            refundMethod: exchangeRefundMethod.value,
            proofReference: exchangeProofKey.value || undefined,
          }
        : {}),
    });

    await reservationApi.changeProduct(props.reservation.id, payload);
    confirmVisible.value = false;
    detailVisible.value = false;
    resetFields();
