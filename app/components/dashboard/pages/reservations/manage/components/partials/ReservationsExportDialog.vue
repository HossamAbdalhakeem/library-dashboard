    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-2 text-right">
        ...
        <Select
          ...
          :options="RESERVATION_EXPORT_STATUS_OPTIONS"
          option-label="label"
          option-value="value"
...
defineOptions({ name: "ReservationsExportDialog" });
...
onMounted(async () => {
  ...
  if (filters.period === "year" && academicYearRange.value?.from) {
    onPeriodChange({
      period: "year",
      from: String(academicYearRange.value.from).slice(0, 10),
      to: String(academicYearRange.value.to).slice(0, 10),
    });
  }
});
...
const onExport = async () => {
  const ok = await exportReservations();
  if (ok) {
